...
You may need to provide an index (default to 1). e.g. if your template has 2 body placeholder, you will need to specify that your content is to be positioned as the 2nd body of your layout (then use index 2). Le paramètre index fonctionne par type.
...
![](caution.png)
Attention : la variable `id` ne correspond pas au paramètre `index` des fonctions `ph_with_*`.
...
Warning : quand le type n'est pas unique, il faut utiliser le paramètre `id_chr` qui correspond à la variable `id` qui se trouve à l'aide de la fonction `layout_properties()`. 
